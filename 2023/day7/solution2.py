...
def get_hand_strength(cards):
    j_cards = cards.count('J')
    cards = cards.replace('J', '')
    if j_cards == 5:
        return 6
    # 5 of a kind
    if cards.count(cards[0]) + j_cards == 5:
        return 6
    # 4 of a kind
    if cards.count(cards[0]) + j_cards == 4 or cards.count(cards[1]) + j_cards == 4:
        return 5
    # Full house options and 3 of a kind
    if cards.count(cards[0]) + j_cards == 3:
        cards = cards.replace(cards[0], '')
        if cards[0] == cards[1]:
            return 4
        else:
            return 3
    if cards.count(cards[1]) + j_cards == 3:
        cards = cards.replace(cards[1], '')
        if cards[0] == cards[1]:
            return 4
        else:
            return 3
    if cards.count(cards[2]) + j_cards == 3:
        ...
        if cards[0] == cards[1]:
            ...
            return 3
    # 2 pair and 1 pair
    if cards.count(cards[0]) + j_cards == 2:
        cards = cards.replace(cards[0], '')
        if cards.count(cards[0]) == 2 or cards.count(cards[1]) == 2:
            return 2
        else:
            return 1
    if cards.count(cards[1]) + j_cards == 2:
        cards = cards.replace(cards[1], '')
        if cards.count(cards[0]) == 2 or cards.count(cards[1]) == 2:
            return 2
        else:
            return 1
    if cards.count(cards[2]) + j_cards == 2:
        cards = cards.replace(cards[2], '')
        if cards.count(cards[0]) == 2 or cards.count(cards[1]) == 2:
            return 2
        else:
            return 1
    if cards.count(cards[3]) + j_cards == 2:
...
with open("input1.in", "r") as file:
    ...
    order = "J23456789TQKA"
    five_bucket = sorted(five_bucket, key=lambda word: [order.index(letter) for letter in word[0]])
    four_bucket = sorted(four_bucket, key=lambda word: [order.index(letter) for letter in word[0]])
    full_bucket = sorted(full_bucket, key=lambda word: [order.index(letter) for letter in word[0]])
    three_bucket = sorted(three_bucket, key=lambda word: [order.index(letter) for letter in word[0]])
    two_pair_bucket = sorted(two_pair_bucket, key=lambda word: [order.index(letter) for letter in word[0]])
    one_pair_bucket = sorted(one_pair_bucket, key=lambda word: [order.index(letter) for letter in word[0]])
    high_card_bucket = sorted(high_card_bucket, key=lambda word: [order.index(letter) for letter in word[0]])
    if DEBUG:
